...
def main():
    ...
    p_geocode = sub.add_parser("geocode", help="只查城市维度(经纬度)")
    p_geocode.add_argument("--city", required=True, help="城市中文名，如 宁德")
    p_weather = sub.add_parser("weather", help="根据经纬度查天气")
    p_weather.add_argument("--lat", type=float, required=True, help="纬度，如 26.66")
    p_weather.add_argument("--lon", type=float, required=True, help="经度，如 119.52")
    p_weather.add_argument("--name", default="该坐标", help="城市名(可选，仅用于报告标题)")
    p_weather.add_argument("--admin1", default="", help="省级(可选)")
    p_weather.add_argument("--country", default="", help="国家(可选)")
    ...
    if args.cmd == "search":
        ...
        print(json.dumps(dim.__dict__, ensure_ascii=False) if not isinstance(dim, str) else dim)
    elif args.cmd == "weather":
        dim = CityDimension(name=args.name, latitude=args.lat, longitude=args.lon,
                            admin1=args.admin1, country=args.country)
