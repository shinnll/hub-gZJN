...
async def run(client, model: str, question: str,
              tool_registry: dict, openai_tools: list[dict], verbose: bool = True, max_rounds: int = 5) -> dict:
    """多轮闭环：提问 → 模型输出 tool_call → 路由到 Server 执行 → 回填 → 若模型继续
    依赖上一步结果调新工具（如 geocode_city → get_weather_from_dimension），则再跑一轮，
    ...
    for _ in range(max_rounds):
        ...
        # 模型输出 tool_calls → 路由到对应 Server 执行，结果以 role=tool 回填
        messages.append(msg)
