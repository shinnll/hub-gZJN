...
NAMED_COMMANDS = {
    ...
    "geocode_city": {  # ① 只查城市维度(经纬度)
        "argv": FINCLI_ARGV + ["geocode"],
        "arg_map": {"city": "--city"},
    },
    "weather": {  # ② 根据经纬度查天气（lat/lon 必填；name/admin1/country 来自 geocode 结果）
...
NAMED_TOOLS_SCHEMA = [
    {
        ...
        "function": {
            ...
            "description": (
                ...
                "可查 A 股年报（rag_search/list_companies）和天气（geocode_city/weather）。"
            ),
            "parameters": {
                "type": "object",
                "properties": {
                    "command": {
                        ...
                        "description": "rag_search（查年报，需 query+可选 stock_code/year/top_k）/"
                                       ...
                                       " geocode_city（只查城市经纬度，需 city）/ weather（按经纬度查天气，需 lat+lon，可选 name/admin1/country）",
                    },
                    "args": {
                        "type": "object",
                        "description": "命令参数。rag_search: {query, stock_code?, year?, top_k?}; geocode_city: {city}; weather: {lat, lon, name?, admin1?, country?}",
                    },
                },
                "required": ["command"],
            },
...
BASH_TOOLS_SCHEMA = [
    {
        ...
        "function": {
            ...
            "description": (
                ...
                "fincli geocode --city 宁德（只查经纬度）；fincli weather --lat 26.66 --lon 119.52（按经纬度查天气）。"
...
SYSTEM_PROMPT_NAMED = (
    ...
    "查天气需两步：① run_cli(command='geocode_city', args={'city': '宁德'}) 拿到经纬度；"
    "② 把返回的 lat/lon（及 name/admin1/country）传给 run_cli(command='weather', args={...}) 查天气。"
...
SYSTEM_PROMPT_BASH = (
    ...
    "查天气：fincli geocode --city 宁德 + fincli weather --lat 26.66 --lon 119.52。"
